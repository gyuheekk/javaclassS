...
<head>
  ...
  <script>
	  $(document).ready(function(){
			...
			//채팅 서버 주소
		  let url = "ws://192.168.50.60:9090/javaclassS/chatserver";
		  ...
		  $('#btnConnect').click(function() {
	     	...
	     	if ($('#user').val().trim() != '') {
	  	   	...
	  	   	// 소캣 이벤트 매핑처리(웹소캣에 연결되면 onopen 메소드 수행처리한다.)
	  	   	ws.onopen = function (evt) {
	  			...
	  			ws.onmessage = function (evt) {					// 서버에 넘어온값이 '2#user명: 메세지' || '2#user명: 메세지#__@__'
		  			...
		  			let no = evt.data.substring(0, 1);		// 접속자가 1(처음 접속자), 2(기존 접속자), 3(종료 접속자) 인지 판별
		  			let user = evt.data.substring(2, index);  // 로그인 사용자의 '유저명'을 가져온다.
		  			...
		  			if (no == '1') { // 최초 접속자는 print2() 메소드 호출	
		  				print2(user);
		  			} else if (no == '2') {	// 기존 접속자들은 채팅중이기에 print()에 '유저명','메세지'를 보낸다.
		  				if (txt != '') print(user, txt);
		  			} else if (no == '3') {	 // 종료사용자는 print3()으로 보낸다
		  				print3(user);
		  			}
		  			$('#list').scrollTop($('#list').prop('scrollHeight')); // 스크롤 바를 가장 아래쪽으로 내린다.	
		  		};
